...
class Sentiment_Analysis:
    ...
    def load_data(self):
        df = pd.read_csv("C:/Users/User/OneDrive - 공주대학교/바탕 화면/review/label_ver2.csv")
    ...
    def vectorize(self, data):
        okt = Okt()
        self.tfidf = TfidfVectorizer(ngram_range=(1, 2), min_df=3, max_df=0.9, tokenizer=okt.phrases, token_pattern=None)
        self.tfidf.fit(data)
    ...
    def start_test(self, test_review):
       ...
       # percent = float(self.rsv_okt.best_estimator_.predict(review_okt))
       # print("{:.2f}\n".format(percent * 100))
        #if score >=0:
        #    result = '긍정 리뷰'
        #elif score == -1:
